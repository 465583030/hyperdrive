...
func (rc *Node) loadSnapshot() *raftpb.Snapshot {
	...
	if err != nil && err != snap.ErrNoSnapshot {
		log.Fatalf("hyperdrive: error loading snapshot (%v)", err)
	}
	return snapshot
}
...
func (rc *Node) openWAL(snapshot *raftpb.Snapshot) *wal.WAL {
	if !wal.Exist(rc.waldir) {
		if err := os.Mkdir(rc.waldir, 0750); err != nil {
			log.Fatalf("hyperdrive: cannot create dir for wal (%v)", err)
		}

		w, err := wal.Create(rc.waldir, nil)
		if err != nil {
			log.Fatalf("hyperdrive: create wal error (%v)", err)
		}
		w.Close()
	}
	...
	if err != nil {
		log.Fatalf("hyperdrive: error loading wal (%v)", err)
	}
...
func (rc *Node) replayWAL() *wal.WAL {
	...
	if err != nil {
		log.Fatalf("hyperdrive: failed to read WAL (%v)", err)
	}
...
func (rc *Node) startRaft() {
	if !fileutil.Exist(rc.snapdir) {
		if err := os.Mkdir(rc.snapdir, 0750); err != nil {
			log.Fatalf("hyperdrive: cannot create dir for snapshot (%v)", err)
		}
	}
...
func (rc *Node) serveRaft() {
	...
	if err != nil {
		log.Fatalf("hyperdrive: Failed parsing URL (%v)", err)
	}

	ln, err := newStoppableListener(url.Host, rc.httpstopc)
	if err != nil {
		log.Fatalf("hyperdrive: Failed to listen rafthttp (%v)", err)
	}
